...
#--* pakages*--
from data_reader import get_price
import pandas as pd
import numpy as np
import scipy as sp
...
class Generator:
    def __init__(self, end_date):
        price = get_price(end_date)

        self.open_price = price['open_price']
        self.high = price['high']
        self.low = price['low']
        self.close = price['close']
        self.avg_price = price['avg_price']
        self.prev_close = price['prev_close']
    ...
    def alpha_114(self):
        ...
        data2 = (((self.high - self.low) / (pd.rolling_sum(self.close, window=5) / 5)) / (self.avg_price-self.close).fillna(pow(10,-50))).iloc[-1, :]
